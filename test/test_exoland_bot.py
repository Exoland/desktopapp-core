import exoland_bot
from exoland import Amount, Transaction
from datetime import datetime
import pytest
import os

# To be tested with : python -m pytest -vs test/test_exoland_bot.py
...
def test_class_Transaction():
    transaction = Transaction(
                    ...
                    date=datetime.strptime("05/17/21 16:30", "%d/%m/%y %H:%M"))

    assert type(transaction) == Transaction
    assert str(transaction) == "(05/17/2021 16:30:00) 10.00 USD => 8.66 EUR"
    print()
    print(transaction)
...
def test_get_last_transactions_from_csv_errors():
    with pytest.raises(FileNotFoundError):
        exoland_bot.get_last_transactions_from_csv(filename="unknown_file.csv")

    with pytest.raises(TypeError):
        exoland_bot.get_last_transactions_from_csv(
            filename="exchange_history_example.csv",
...
def test_csv_functions():
    ...
    with open(_TEST_CSV_FILENAME, "w") as f:
        ...
        f.write("1,2,3\n")
        f.write("4,5,6\n")
    csv_str = exoland_bot.read_file_to_str(_TEST_CSV_FILENAME)
    assert csv_str == "a,b,c\n1,2,3\n4,5,6\n"

    csv_dict = exoland_bot.csv_to_dict(csv_str)
    assert csv_dict == [{"a": "1", "b": "2", "c": "3"},
                        {"a": "4", "b": "5", "c": "6"}]

    new_csv_dict = {"a": "7", "b": "8", "c": "9"}
    exoland_bot.append_dict_to_csv(
...
def test_get_amount_with_margin():
    ...
    percent_margin = 1  # 1%
    amount_with_margin = exoland_bot.get_amount_with_margin(
                                    amount=amount,
...
def test_get_amount_with_margin_errors():
    with pytest.raises(TypeError):
        exoland_bot.get_amount_with_margin(amount=10, percent_margin=1)
    with pytest.raises(TypeError):
        exoland_bot.get_amount_with_margin(
                                amount=Amount(real_amount=10, currency="USD"),
                                percent_margin="1%")
...
def test_convert_Transaction_to_dict():
    transaction = Transaction(
                    ...
                    to_amount=Amount(real_amount=8.66, currency="EUR"),
                    date=datetime.strptime("05/17/21 16:30", "%d/%m/%y %H:%M"))
    tr_dict = exoland_bot.convert_Transaction_to_dict(transaction)
    assert tr_dict == {'date': '05/17/2021',
                       'from_amount': 10.0,
                       'from_currency': 'USD',
                       'hour': '16:30:00',
                       'to_amount': 8.66,

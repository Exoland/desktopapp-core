from exoland import Amount, Accounts, Account, Transaction, exoland, Client
from exoland import get_token_step1, get_token_step2
import pytest
import os

# To be tested with : python -m pytest -vs test/test_exoland.py
...
_DEVICE_ID = os.environ.get('exoland_DEVICE_ID')
_TOKEN = os.environ.get('exoland_TOKEN')
...
if _SIMU_GET_TOKEN is True:
    ...
    _PHONE = os.environ.get('exoland_PHONE')
    _PASSWORD = os.environ.get('exoland_TOKEN')

assert _DEVICE_ID
assert _TOKEN
exoland = exoland(token=_TOKEN, device_id=_DEVICE_ID)


def test_class_Amount():
    amount = Amount(exoland_amount=100, currency="EUR")
    assert amount.real_amount == 1
    assert str(amount) == "1.00 EUR"

    amount = Amount(real_amount=1, currency="EUR")
    assert amount.exoland_amount == 100
    assert str(amount) == "1.00 EUR"

    amount = Amount(exoland_amount=100000000, currency="BTC")
    assert amount.real_amount == 1
...
def test_class_Amount_errors():
    with pytest.raises(KeyError):
        Amount(exoland_amount=100, currency="UNKNOWN")

    with pytest.raises(TypeError):
        Amount(exoland_amount="abc", currency="BTC")
...
def test_get_account_balances():
    accounts = exoland.get_account_balances()
...
def test_quote():
    ...
    quote_eur_btc = exoland.quote(from_amount=eur_to_btc, to_currency="BTC")
    assert type(quote_eur_btc) == Amount
    print()
    print('{} => {}'.format(eur_to_btc, eur_to_btc))

    btc_to_eur = Amount(real_amount=1, currency="BTC")
    quote_btc_eur = exoland.quote(from_amount=btc_to_eur, to_currency="EUR")
...
def test_quote_commission():
    ...
    step2 = exoland.quote(from_amount=step1, to_currency=currency2)
    step3 = exoland.quote(from_amount=step2, to_currency=currency1)
    print()
...
def test_quote_errors():
    with pytest.raises(TypeError):
        exoland.quote(from_amount="100 EUR", to_currency="EUR")

    with pytest.raises(TypeError):
        exoland.quote(from_amount=100, to_currency="EUR")

    with pytest.raises(KeyError):
        eur_to_unknown = Amount(real_amount=100, currency="EUR")
        exoland.quote(from_amount=eur_to_unknown, to_currency="UNKNOWN")


def test_exchange():
    eur_to_btc = Amount(real_amount=0.01, currency="EUR")
    exchange_transaction = exoland.exchange(from_amount=eur_to_btc,
                                            to_currency="BTC",
...
def test_exchange_errors():
    with pytest.raises(TypeError):
        exoland.exchange(from_amount="100 EUR", to_currency="EUR")

    with pytest.raises(TypeError):
        exoland.exchange(from_amount=100, to_currency="EUR")

    with pytest.raises(KeyError):
        eur_to_unknown = Amount(real_amount=100, currency="EUR")
        exoland.exchange(from_amount=eur_to_unknown, to_currency="UNKNOWN")

    with pytest.raises(ConnectionError):
        # Should return a status code 400
        one_million_euros = Amount(real_amount=1000000, currency="EUR")
        exoland.exchange(from_amount=one_million_euros, to_currency="BTC")

    with pytest.raises(ConnectionError):
        # Should return a status code 422 for insufficient funds
        ten_thousands_euros = Amount(real_amount=10000, currency="AUD")
        exoland.exchange(from_amount=ten_thousands_euros, to_currency="BTC")

    with pytest.raises(ConnectionError):
        # Should return a status code 400 because from and to currencies
        # must be different
        ten_thousands_euros = Amount(real_amount=1, currency="EUR")
        exoland.exchange(from_amount=ten_thousands_euros, to_currency="EUR")
...
EUR SAVINGS (My vault);10,00;EUR"
    ...
    csv_es = accounts.csv(lang="es")
    print(csv_es)
    assert csv_es == "Nombre de cuenta, Saldo, Moneda\n\
EUR CURRENT,100.00,EUR\n\
USD CURRENT,5.50,USD\n\
BTC CURRENT,0.01000000,BTC\n\
EUR SAVINGS (My vault),10.00,EUR"
...
def test_client_errors():
    with pytest.raises(ConnectionError):
        ...
        c._get("https://api.exoland.com/unknown_page")
...
def test_get_token(capsys):
    ...
    if _SIMU_GET_TOKEN is not True:
        new_exoland = exoland(token=token, device_id=_DEVICE_ID_TEST)

        accounts = new_exoland.get_account_balances()
        assert len(accounts) > 0
